...
for route in files:
    try:
        ...
        results = results.append(pd.Series([route, chosen_features,'train', (abs(scores['test_abs_error'].mean())), (math.sqrt(abs(scores['test_squared_error'].mean()))), (scores['test_r2'].mean()), datetime.today().strftime('%Y-%m-%d')], index=results.columns ), ignore_index=True)
        results.to_csv('LRScores.csv', mode='a', header=False, index=False)
